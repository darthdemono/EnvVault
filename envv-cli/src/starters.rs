...
pub fn starter_chunks(ptype: &str) -> Option<Vec<Value>> {
    match ptype {
        "wireguard" => Some(vec![
            chunk("Interface", "wg_interface", vec![
                fs("PrivateKey", ""),
                f("Address", "", "var"),
                f("MTU", "", "var"),
                f("Table", "", "var"),
        ...
        "nginx" => Some(vec![
            ...
            chunk("HTTPS :443 main", "nginx_server", vec![
                f("listen", "443 ssl http2", "port"),
                fd("listen", "[::]:443 ssl http2", "port", "ipv6"),
            ...
            chunk("location /", "nginx_location", vec![
                f("path", "/", "var"),
                f("try_files", "$uri $uri/ $uri.php?$args", "var"),
            ]),
            chunk("location ~ .php", "nginx_location", vec![
                f("path", "~ \\.php$", "var"),
                f("include", "snippets/fastcgi-php.conf", "var"),
                f("fastcgi_pass", "unix:/run/php/php8.1-fpm.sock", "endpoint"),
            ]),
            chunk("location ~ assets", "nginx_location", vec![
                f("path", "~* \\.(jpg|jpeg|png|gif|webp|ico|css|js|svg|woff2)$", "var"),
                f("expires", "30d", "var"),
