...
export function exportNginx(project: Project): string {
  ...
  for (const s of servers) {
    lines.push('server {');
    for (const f of s.fields) { if (f.value) lines.push(`    ${f.key} ${rv(f.value)};`); }
    if (locations.length) {
      lines.push('');
      for (const loc of locations) {
        const path = loc.fields.find(f => f.key === 'path')?.value || '/';
        lines.push(`    location ${path} {`);
        for (const f of loc.fields) { if (f.key !== 'path' && f.value) lines.push(`        ${f.key} ${rv(f.value)};`); }
        lines.push('    }');
      }
    }
    lines.push('}', '');
