/**
 * @file ENV chunk <-> vault linking — match plain `KEY=value` env-file fields
...
function _findBestVaultMatch(key: string, value: string): { entry: VaultEntry; ref: string; field: string; confidence: number } | null {
  ...
  for (const e of st.vault.api_keys) {
    ...
    for (const pfx of (e.env_prefixes ?? [])) {
      ...
      const xv = e.extra_vars?.find(v =>
        v.key === stripped || v.key.toUpperCase() === stripped.toUpperCase());
      if (xv) {
        score = 92; field = xv.key; break;
      } else {
        ...
        if (stripped.toUpperCase().startsWith(provKey)) {
          ...
          score = 92; field = suffix || 'key'; break;
        }
      }
    }
    ...
    if (score === 0 && value.length >= 6) {
      const candidates: Array<[string, string | null | undefined, number]> = [
        ['key',      e.api_key,    88],
      ...
      for (const [fName, fVal, fScore] of candidates) {
        if (fVal && value === fVal) { score = fScore; field = fName; break; }
      }
    }
    ...
    if (score === 0) {
      ...
      for (let i = 1; i < parts.length && score === 0; i++) {
        ...
        if (stripped === e.provider) { score = 75; field = 'key'; }
        else if (e.key_id && stripped === `${e.provider}_${e.key_id}`) { score = 70; field = 'key'; }
        else if (stripped.toLowerCase() === e.provider.toLowerCase()) { score = 63; field = 'key'; }
      }
    }
...
function _suggestCreateEntry(key: string): { provider: string; keyId?: string; secretType: SecretType } {
  ...
  else if (/[_-](url|uri|dsn|conn(ection)?(_?str(ing)?)?)$/.test(k)) secretType = 'connection_string';

  const suffixMatch = key.match(/^(.+)[_-](API_?KEY|APIKEY|KEY|TOKEN|SECRET|PASS(WORD)?|PWD|USER(NAME)?|LOGIN|URL|URI|DSN)$/i);
...
export function buildEnvLinkMatches(chunk: SecretChunk): EnvLinkMatch[] {
  return chunk.fields.filter(f => f.key).map(f => {
    const refM = f.value?.match(/^\$\{(.+)\}$/);
    ...
    return { key: f.key, rawValue: f.value, alreadyLinked: false, suggestCreate: _suggestCreateEntry(f.key) };
  });
}
