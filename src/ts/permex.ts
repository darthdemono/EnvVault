...
export function globMatches(pattern: string, value: string): boolean {
  ...
  while (vi < value.length) {
    if (pi < pattern.length && pattern[pi] === '*') {
      ...
      starVi = vi;
      pi++;
    } else if (pi < pattern.length && (pattern[pi] === value[vi] || pattern[pi] === '?')) {
      pi++;
      vi++;
    } else if (starPi !== -1) {
      pi = starPi + 1;
...
function lex(src: string): Tok[] {
  ...
  while (i < src.length) {
    ...
    if (!glob)
      throw new Error(`field '${word}' has no value — write ${word}:* to match everything`);
...
export function evaluate(expr: Expr, entry: VaultEntry, projects: Project[]): boolean {
  switch (expr.kind) {
    case 'and':
      return evaluate(expr.a, entry, projects) && evaluate(expr.b, entry, projects);
    case 'or':
      return evaluate(expr.a, entry, projects) || evaluate(expr.b, entry, projects);
    case 'not':
      return !evaluate(expr.a, entry, projects);
    case 'pred': {
      ...
      switch (field) {
        case 'category':
          return (entry.categories ?? []).some((c) => globMatches(glob, c));
        case 'tag':
          return (entry.tags ?? []).some((t) => globMatches(glob, t));
        case 'env':
          return !!entry.environment && globMatches(glob, entry.environment);
        case 'type':
          return globMatches(glob, entry.secretType ?? 'api_key');
        case 'project':
...
export function evaluateSrc(src: string, entry: VaultEntry, projects: Project[]): boolean {
  try {
    return evaluate(parse(src), entry, projects);
  } catch {
    return false;
  }
}
