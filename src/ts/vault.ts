    ...
    if (data) {
      ...
      if (remoteEmpty && localHasData && st.store.isRemote) {
        ...
        st.vault.projects = data.projects || [
          { id: 'Universal', name: 'Universal', description: '' },
        ];
      }
      ...
      for (const key of st.vault.api_keys) {
        if (!key.projectIds) {
          key.projectIds = ['Universal'];
          needsSave = true;
        } else if (!key.projectIds.includes('Universal')) {
          key.projectIds.push('Universal');
          needsSave = true;
        }
        if (!key.secretType) {
        ...
        if ((key.secretType as string) === 'chunk') {
          key.secretType = 'env_var';
          needsSave = true;
        }
      }
...
async function init() {
  ...
  document
    ...
    ?.addEventListener('click', () => openFilePicker('.json,.env,text/plain'));
  ...
  document
    .getElementById('f-icon-pick')!
    .addEventListener('click', () =>
      openIconPicker(
        document.getElementById('f-icon') as HTMLInputElement,
        document.getElementById('f-icon-preview') as HTMLElement,
      ),
    );
  ...
  document.getElementById('copy-all-arrow')!.addEventListener('click', (e) =>
    showDropdown(e.currentTarget as HTMLElement, [
  ...
  document
    ...
    .addEventListener('click', () => copyAll(Settings.get('defaultExportFormat')));
  ...
  document
    .getElementById('env-import-select-all')
    ?.addEventListener('click', () =>
      document
        .querySelectorAll<HTMLInputElement>('.env-import-check')
        .forEach((cb) => (cb.checked = true)),
    );
  document
    .getElementById('env-import-select-none')
    ?.addEventListener('click', () =>
      document
        .querySelectorAll<HTMLInputElement>('.env-import-check')
        .forEach((cb) => (cb.checked = false)),
    );
  document.getElementById('env-import-overlay')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeEnvImportModal();
  });
  ...
  document.getElementById('env-link-overlay')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeEnvLinkModal();
  });
  ...
  document.getElementById('settings-import-encrypted')?.addEventListener('click', () => {
    const pw = (document.getElementById('settings-backup-pw') as HTMLInputElement).value;
    if (!pw) {
      showToast('Enter the backup password first', 'err');
      return;
    }
    const inp = document.createElement('input');
    inp.type = 'file';
    inp.accept = '.vaultbak,.json,text/plain';
    inp.onchange = () => {
      const f = inp.files?.[0];
      if (!f) {
        inp.remove();
  ...
  document
    .getElementById('new-category-root-btn')
    ?.addEventListener('click', openCategoryCreateModal);
  document
    .getElementById('category-create-close')
    ?.addEventListener('click', closeCategoryCreateModal);
  document
    .getElementById('category-create-cancel')
    ?.addEventListener('click', closeCategoryCreateModal);
  document.getElementById('category-create-save')?.addEventListener('click', saveCategoryCreate);
  document.getElementById('category-create-overlay')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeCategoryCreateModal();
  });
  ...
  document
    .getElementById('project-create-close')
    ?.addEventListener('click', closeProjectCreateModal);
  document
    .getElementById('project-create-cancel')
    ?.addEventListener('click', closeProjectCreateModal);
  ...
  document.getElementById('project-create-overlay')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeProjectCreateModal();
  });
  ...
  document.getElementById('chunk-edit-overlay')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) closeChunkEditModal();
  });
  ...
  document
    ...
    .addEventListener('click', () => shortcutsEl.classList.add('open'));
  document
    .getElementById('shortcuts-close')!
    .addEventListener('click', () => shortcutsEl.classList.remove('open'));
  ...
  document.getElementById('card-grid')!.addEventListener('click', (e) => {
    ...
    if (
      action === 'copy-env' &&
      el.classList.contains('card-head') &&
    ...
    switch (action) {
      case 'toggle':
        ...
        break;
      case 'reveal':
        toggleReveal(e, field, idx, value);
        break;
      case 'copy-field':
        copyField(e, value, el);
        break;
      case 'copy-env':
        doCopyEnv(e, idx);
        break;
  ...
  document.getElementById('card-grid')!.addEventListener('contextmenu', (e) => {
    ...
    if (entry.api_secret)
      _items.push({
        label: 'Copy Secret',
        fn: () => copyField(e as MouseEvent, entry.api_secret!, card),
      });
    ...
    const onEntry = (fn: (i: number) => void) => () => {
      const i = liveIdx();
      if (i < 0) {
        showToast('That secret no longer exists', 'err');
        return;
      }
      fn(i);
    };
    ...
    const items: any[] = [
      {
        ...
        fn: () => {
          if (isRemote) void switchToLocalVault();
        },
      },
      '---',
    ];
