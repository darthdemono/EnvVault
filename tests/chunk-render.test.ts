...
describe('chunkToString — nginx', () => {
  ...
  it('resolves a certificate reference', () => {
    st.vault = makeVault({
      api_keys: [makeEntry({ provider: 'CertPath', api_key: '/etc/ssl/full.pem' })],
    });
    const out = chunkToString(
      chunk({
        name: 'srv',
      ...
      chunks: [
        chunk({
          ...
          fields: [
            ...
            {
              key: 'ssl_certificate',
              value: '/etc/letsencrypt/live/other.org/fullchain.pem',
              field_type: 'cert',
            },
          ],
        }),
...
describe('certEntryForDomain / ensureCertForDomain', () => {
  ...
  it('ignores a non-certificate entry with the same name', () => {
    st.vault = makeVault({
      api_keys: [makeEntry({ provider: 'example.com', secretType: 'api_key' })],
    });
...
describe('redundantCertKeyChunkIds', () => {
  it('flags an nginx_key chunk whose PEM duplicates the vault entry', () => {
    ...
    st.vault = makeVault({
      api_keys: [
        makeEntry({ provider: 'example.com', secretType: 'certificate', certificate_data: pem }),
      ],
    });
    const proj = makeProject({
      id: 'p1',
  ...
  it('does not flag a chunk holding different content', () => {
    st.vault = makeVault({
      api_keys: [
        makeEntry({ provider: 'example.com', secretType: 'certificate', certificate_data: 'AAA' }),
      ],
    });
    const proj = makeProject({
      id: 'p1',
      ...
      chunks: [
        chunk({
          ...
          name: 'k',
          chunk_type: 'nginx_key',
          fields: [{ key: 'content', value: 'BBB', field_type: 'cert' }],
        }),
        chunk({
          id: 'srv',
          name: 'srv',
    ...
    const proj = makeProject({
      ...
      chunks: [
        chunk({
          ...
          name: 'k',
          chunk_type: 'nginx_key',
          fields: [{ key: 'content', value: 'AAA', field_type: 'cert' }],
        }),
      ],
    } as any);
    expect(redundantCertKeyChunkIds(proj)).toEqual([]);
...
describe('renderChunkCard', () => {
  ...
  it('masks a secret field rather than printing it', () => {
    const card = renderChunkCard(
      chunk({
        name: 'Interface',
        chunk_type: 'wg_interface',
        fields: [
          { key: 'PrivateKey', value: 'SUPER_SECRET_KEY', field_type: 'secret', secret: true },
        ],
      }),
      proj(),
    );
  ...
  it('marks a disabled chunk', () => {
    const card = renderChunkCard(
      chunk({ name: 'x', chunk_type: 'env_file', disabled: true }),
      proj(),
    );
